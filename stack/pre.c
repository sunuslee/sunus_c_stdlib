...
int _get_stack(struct _stack *stack_name, int elem_nr, size_t elem_size)
{
        ...
        do { stack_name->button = (void *)malloc(size); if(!stack_name->button) printf("In malloc" "Out Of Memory\n"); }while(0);
...
int main()
{
        int i = 0;
        int *p;
        struct _stack *st; do { do { st = (struct _stack *)malloc(sizeof(struct _stack)); if(!st) printf("In malloc" "Out Of Memory\n"); }while(0); if(_get_stack(st, 5, sizeof(int)) == 0) printf("get stack succeed!\n"); else printf("get stack fail!\n"); }while(0);
        for(i = 0 ; i < 6 ; i++)
        {
                do { if(((uint32_t)(st->sp) + (uint32_t)(st->elem_size)) > (uint32_t)st->top) { printf("ERROR : sp + elem_size > top\n"); break; } st->sp += st->elem_size; memmove(st->sp, (void *)&i, st->elem_size); }while(0);
        }
        for(i = 0; i < 6 ; i++)
        {
                p = (int *)(st->sp - st->elem_size) >= st->button ? st->sp -= st->elem_size , (void *)(st->sp + st->elem_size) : ((void *)0);
                printf("p = %08x\n",(uint32_t)p);
                if(p != ((void *)0))
                        printf("*p = %d poped\n",*p);
                else
