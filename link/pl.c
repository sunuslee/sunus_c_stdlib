...
int main()
{
        ...
        int i;
        do { do { phead = (struct link_list *)calloc(1,sizeof(struct link_list)); if(!phead) printf("In calloc" "Out Of Memory\n"); }while(0); phead->plink.free_addr = phead; phead->pdata = (void *)(head ? head : ((void *)0)) ; }while(0);
        for (i = 0 ; i < 10 ; i++)
        {
                do { do { newnode = (struct link_list *)calloc(1,sizeof(struct link_list)); if(!newnode) printf("In calloc" "Out Of Memory\n"); }while(0); newnode->plink.free_addr = newnode; newnode->pdata = (void *)(data[i] ? data[i] : ((void *)0)) ; }while(0);
                do { (phead->plink).prev = data[i]; (data[i]->plink).next = phead; }while(0);
        }
        printf("CHECK 1:\n\n");
        for(i = 0 ,pcheck = phead; i < 10 ; i++)
        {
                printf("Node data : %s",(char *)pcheck->pdata);
                pcheck = (pcheck->plink).next;
                if(pcheck == ((void *)0))
